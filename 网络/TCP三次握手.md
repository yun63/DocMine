...
1. connect
   ...
   客户端在调用connect函数之前不必非得调用bind函数，因为内核会去订阅IP地址，并按照一定的算法选择一个临时端口作为源端口。
...
1. 两次握手会产生什么问题？
   ...
   **过程**：
   ...
   2. 服务器S发送同步信号SYN k并对客户端C发送确认信号ACK j+1
